<template>
    <div class="titlewpic">

<div class="title">Availability</div>
<img
            v-for="(image, key) in imagess"
            v-show="key ? key.includes($route.params.id) : false"
            :src="image.url"
            :alt="key"
            :style="image.style"
            :class="image.class"
        >
</div>

<div class="floatr">

    <img
            v-for="(image, key) in images"
            v-show="key ? key.includes($route.params.id) : false"
            :src="image.url"
            :alt="key"
            :style="image.style"
            :class="image.class"
        >
</div>
    <!-- <h2 class="chapter">* Available colors and models.</h2>

<h3 class="apraksts">iPhone 15 and iPhone 15 Plus is available in 5 colors which include
     - Black, Blue, Green, Yellow and Pink!
</h3>
<h3 class="apraksts">iPhone 15 Pro and iPhone 15 Pro Max will be available
     in four stunning new finishes, including black titanium, white titanium,
      blue titanium, and natural titanium.
</h3> -->

        <div class="spacing"></div>
        <div class="chapter">{{ title[$route.params.id] }}</div>
        <div class="spacing"></div>
        <div class="apraksts">{{ text[$route.params.id] }}</div>
        <div class="spacing"></div>
        <div class="apraksts">{{ text2[$route.params.id] }}</div>

</template>

<script setup>
import { ref, reactive } from 'vue';

const imagess = reactive({
    "15" : {
        url: 'https://shorturl.at/amqGR',
        style: {
            height: '200px',
            width: '200px',
        },
        class: 'hhhh',
    },
    "14": {
        url: 'https://www.apple.com/newsroom/images/product/iphone/standard/Apple-iPhone-14-iPhone-14-Plus-2up-blue-220907_inline.jpg.large.jpg',
        style: {
            height: '200px',
            width: '160px',
        },
        class: 'hhhh',
    },
    "13": {
        url: 'data:image/jpeg;base64,',
        style: {
            height: '200px',
            width: '160px',
        },
        class: 'hhhh',
    },
});

const images = reactive({
    "15": {
        url: 'https://pbs.twimg.com/media/F4s5EeTaMAARyOU?format=jpg&name=4096x4096',
        style: {
            height: '190px',
            width: '340px',
        }
    },
    "14": {
        url: 'https://www.fonehouse.co.uk/blog/wp-content/uploads/2022/09/header-image-iphone14-colours-blog-1-1200x600.jpg',
        style: {
            height: '200px',
            width: '400px',
        }
    },
    "13": {
        url: 'https://img.buzzfeed.com/buzzfeed-static/complex/images/lh4z30okotrbzpwcus1c/iphone-13-06.png',
        style: {
            height: '240px',
            width: '420px',
        }
    },
});

const title = ref({
    15: '* Available colors and models',
    14: '* Available colors and models',
    13: '* Available colors and models',
});

const text = ref({
    15: 'iPhone 15 and iPhone 15 Plus is available in 5 colors which include - Black, Blue, Green, Yellow and Pink!',
    14: 'The iPhone 14 Pro is available in four colours: Silver, Space Black, Gold, and Deep Purple.',
    13: 'iPhone 13 Pro and iPhone 13 Pro Max come in five stunning finishes: sierra blue, graphite, gold, silver, and the all-new alpine green.',
});

const text2 = ref({
    15: 'iPhone 15 Pro and iPhone 15 Pro Max will be available in four stunning new finishes, including black titanium, white titanium, blue titanium, and natural titanium.',
    14: 'iPhone 14 and iPhone 14 Plus come in six stunning colors: midnight, starlight, red, blue, purple, and the all-new yellow.',
    13: 'iPhone 13 and iPhone 13 mini come in six beautiful colours: red, starlight, midnight, blue, pink, and the all-new green.',
});

</script>
<script>
export default {
    
}
</script>
<style lang="scss">
    
</style>